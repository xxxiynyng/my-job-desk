...
### 1. ~~`ProfileCompletionCard` — 미연결 기능~~ → ✅ **해결됨(2026-07-05 추가 작업)**
- 사용자 결정: "삭제하지 말고 두 곳에 컴팩트하게 연결, 완성도 계산은 단일 출처로."
- **구현(커밋 dba6f93·15107da·4487967)**:
  - `hooks/useProfileCompletion` + `data/basicInfoFields`(SSOT) — 완성도(%)·미완 필드 목록을 **한 곳에서 파생**(`specs.info.values.v2` 기준). BasicInfoPanel 헤더도 동일 로직 사용.
  - **대시보드**: 얇은 1줄 배너(`ProfileCompletionBanner`) — 공고 리스트 위, <100%만 노출, dismiss(세션 내)+완료 시 자동 제거, 클릭 시 tab2 이동.
  - **탭2 기본정보**: 상세 카드(`ProfileCompletionCard`) — 게이지+미완 칩+CTA. 칩→해당 필드 인라인 편집, CTA→전체 편집 모달.
  - 온보딩의 구 카드(별도 completeness 모델·pending 큐)는 단일 출처와 충돌해 훅 기반 카드로 대체(컴포넌트는 삭제가 아니라 `components/pickd`로 재탄생).
  - **불변 원칙 유지**: AI가 값을 채우지 않고 사용자가 직접 채우도록 유도만.
...
### 4. 기타 참고
- `.claude/settings.local.json`이 `1fd2965`에 `git add -A`로 혼입됐던 문제 → ✅ **해결(커밋 4671dea)**: gitignore 등록 + 캐시 제거(로컬 파일 유지). 이후 재혼입 없음.
...
**추가 작업 — 프로필 완성도 연결 (2026-07-05 오후):**
```
4671dea chore: .claude/settings.local.json gitignore 추가 — 커밋 재혼입 방지
4487967 feat(profile): 탭2 기본정보 완성도 카드 연결 — 상세형(게이지·미완칩·CTA)
15107da feat(profile): 대시보드 완성도 배너 연결 — 컴팩트 1줄
dba6f93 feat(profile): 완성도 계산 단일 출처(useProfileCompletion) 추가
```
